...
```{r}
data = read.csv("data_train_woMiss.csv")
dim(data)
head(data, 20)
# Class labels:
# 1 - Normal
...
data <- data[,-1:-3]
data <- subset(data, select = c(EDA_Mean, EDA_Min, EDA_Max, EDA_Std, EDA_Kurtosis, EDA_Skew, EDA_Num_Peaks, EDA_Amphitude, EDA_Duration, HR_Mean, HR_Min, HR_Max, HR_Std, HR_RMS, temp_Mean, temp_Min, temp_Max, temp_Std, Stress_level))
summary(data)
...
```{r}
fviz_pca_biplot(pr_scale, habillage = data$Stress_level)
```
...
fviz_pca_biplot(pr_scale,
             col.ind = 'coord',
             habillage = data$Stress_level, label = "var", title = "PCA Biplot of Across Stress Levels") + theme(text=element_text(family="Times New Roman", face="bold", size=20) + theme(legend.position = c(0.8, 0.2)))
ggsave("PCAtrain.png")
```
